...
@Component({
    ...
    <div class="ce-container" id="main-content" tabindex="-1">
      <header class="ce-header glass">
        <div>
          ...
          <p class="ce-sub">Crea la identidad con la que entrarás a los casos. No afecta tu evaluación.</p>
        </div>
        <div class="ce-id">
          <span class="ce-chip">Programa: Psicología</span>
        ...
        <section class="ce-stage glass" aria-label="Vista previa del avatar">
          ...
          <div class="ce-pose" role="radiogroup" aria-label="Pose">
            ...
            <button type="button" class="ce-opt" [class.sel]="pose() === 'side'"
              role="radio" [attr.aria-checked]="pose() === 'side'" (click)="pose.set('side')">3/4</button>
          </div>
          <p class="sr-only">{{ avatarSummary() }}</p>
        </section>

        <!-- DERECHA: resumen + uniforme -->
        <section class="ce-panel glass" aria-label="Uniforme y resumen">
  ...
  styles: [`
    ...
    .ce-grid { display: grid; grid-template-columns: 300px minmax(0,1fr) 300px; gap: 14px; align-items: start; }
    ...
    .ce-group { display: grid; gap: 6px; }
    .ce-label { color: var(--ink-soft); font-size: .76rem; font-weight: 700; }
    .ce-swatches { display: flex; flex-wrap: wrap; gap: 8px; }
    .ce-swatch { width: 30px; height: 30px; border-radius: 50%; border: 2px solid rgba(255,255,255,.18); cursor: pointer; transition: transform .15s, border-color .15s, box-shadow .15s; }
    ...
    .ce-summary strong { color: var(--ink); }
